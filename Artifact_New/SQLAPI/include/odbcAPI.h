...
//! ODBC messages handling callback
typedef void (SQLAPI_CALLBACK* odbcMsgHandler_t)(const SAString& sSqlcode, const SAString& sMessage, void* pAddInfo);

typedef struct SQLAPI_API _odbcMsgCallbackData {
	odbcMsgHandler_t fHandler;
	void* pAddInfo;

	_odbcMsgCallbackData()
	{
		fHandler = NULL;
		pAddInfo = NULL;
	}
...
class SQLAPI_API odbcAPI : public IsaAPI
{
public:
	odbcAPI();
...
public:
	...
	void Check(
		...
		SQLSMALLINT HandleType,
		SQLHANDLE Handle,
		odbcMsgHandler_t fMsgHandler = NULL,
...
protected:
	...
	odbcMsgCallbackData msgCallbackData;
...
public:
	void SetMessageCallback(odbcMsgHandler_t fHandler, void* pAddInfo, SAConnection* pCon = NULL);
};

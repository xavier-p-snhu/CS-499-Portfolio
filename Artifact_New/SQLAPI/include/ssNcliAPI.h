...
//! MSSQL ODBC messages handling callback
typedef void (SQLAPI_CALLBACK* ssMsgHandler_t)(const SAString& sSqlcode, const SAString& sMessage, void* pAddInfo);

typedef struct SQLAPI_API _ssMsgCallbackData {
	ssMsgHandler_t fHandler;
	void* pAddInfo;
...
class SQLAPI_API ssNCliAPI : public IsaAPI
{
public:
	ssNCliAPI();
...
protected:
	...
	ssMsgCallbackData msgCallbackData;
	
	bool ReadDiagRec(SQLSMALLINT HandleType, SQLHANDLE Handle, SQLSMALLINT i, SQLINTEGER& NativeError, SAString& sCode, SAString& sMsg);

public:
	void SetMessageCallback(ssMsgHandler_t fHandler, void* pAddInfo, SAConnection* pCon = NULL);
	...
	void Check(
		...
		SQLSMALLINT HandleType,
		SQLHANDLE Handle,
		ssMsgHandler_t fMsgHandler = NULL,
		void* pMsgAddInfo = NULL);
	static SAString& SQLWCHAR2String(SAString& dst, const SQLWCHAR* szSrc, size_t nSrcLen = SIZE_MAX);
	void AllocConnectionHandle(SQLHDBC*	hdbc);
};
